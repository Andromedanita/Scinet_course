...
int main(int argc, char* argv[])
{
    ...
    for (int s = 0; s < nsteps; s++) {
        ...
        if ((s+1)%nper == 0) {
	     ...
	     int threads;
	     // dataFile is shared between threads since we are writing to one single file
#pragma omp parallel default(none) shared (cout,npnts,x,rho,dataFile) private(threads)
	     {
	       // number of the current thread executing
	       threads = omp_get_thread_num();
	       dataFile << time << "\n";
	       // parallelizing the for loop
             #pragma omp for 
             for (int i = 0; i < npnts; i++ )
               #pragma omp critical // making the parallelization to be critical so that one thread executes at a time
