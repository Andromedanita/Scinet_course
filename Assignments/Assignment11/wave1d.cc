...
{
    int ncid, varXid,varTid,varRid, status, dimRho[2],dimX[1],dimTime[1];
    ...
    // Output file name
    const std::string dataFilename = parameter.get<std::string>("outputname","dataFilecdf.nc");
    ...
    std::cout << "#outputname" << dataFilename << std::endl;
    ...
    if (graphics) {
      ...
      // adding the code for writing the output using netcdf
	status = nc_create(dataFilename.c_str(), NC_NETCDF4, &ncid);
	status = nc_def_dim(ncid, "X", ngrid, &dimRho[0]);
	status = nc_def_dim(ncid, "t", nsteps, &dimRho[1]);
	status = nc_def_var(ncid, "Rho", NC_FLOAT, 2, dimRho, &varRid);
    ...
    for (int s = 0; s < nsteps; s++) {
        ...
        if ((s+1)%nper == 0) {
	     ...
	     status = nc_put_var_float(ncid,varRid,rho.data());
           } 
        }
    }
